...
`v0.1.5` 이후 Jetson에서 sampled benchmark를 반복 실행할 때 확인할 운영 체크리스트를 정한다. 목표는 매번 같은 기준으로 환경을 확인하고, evidence를 보관하고, 실패 시 어디서부터 triage할지 빠르게 판단하는 것이다.
...
tar -xf /tmp/inferedgeenv-v0.1.5-ops.tar -C /tmp/InferEdgeEnv-jetson-ops-source
...
- **Release Rehearsal**: this checklist is the operational companion to the current `v0.1.5` release gate.
...
After `v0.1.5`, the useful next improvement is not more scope but repeatability. Jetson evidence can be convincing only when the environment, run roots, exported bundles, imported registry, and handoff report are collected consistently.
