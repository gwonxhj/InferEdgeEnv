# EdgeEnv Documentation Language Guide

> Language: [English](#english-overview) | [한국어](#korean-overview)

## English Overview

EdgeEnv documentation is maintained with an English-first representative path and Korean/mixed-language working notes.

Use this page when a document links here from its language selector:

- English representative path: start with [README](../README.md), then follow the Guide Map.
- Korean entry path: start with [한국어 README](ko/README.md).
- Source-of-truth behavior remains in code, tests, examples, and the release quality gate.
...
EdgeEnv 문서는 영어 대표 경로와 한국어/혼합 언어 작업 기록을 함께 유지한다.

문서 상단의 language selector에서 이 페이지로 이동했다면 다음 기준으로 읽으면 된다.

- 영어 대표 경로: [README](../README.md)에서 시작한 뒤 Guide Map을 따른다.
- 한국어 진입 경로: [한국어 README](ko/README.md)에서 시작한다.
- 실제 동작의 source of truth는 코드, 테스트, 예제, release quality gate다.
