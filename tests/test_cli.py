...
def test_cli_local_sampler_recoverable_failure_writes_sampler_metadata(tmp_path):
    runner = CliRunner()
    script = tmp_path / "local_bench.py"
    script.write_text(
        """
...
""".strip()
    ...
    assert "Benchmark run stored" in result.output
    assert "Resource metrics: omitted" in result.output
    assert "Sampler metadata: stored" in result.output
    run_dirs = list((edgeenv_root / "runs").iterdir())
    assert len(run_dirs) == 1
    metadata = json.loads(
        (run_dirs[0] / "sampler" / "metadata.json").read_text(encoding="utf-8")
    )
    assert metadata["sample_count"] == 0
    assert "tegrastats unavailable" in metadata["warnings"][0]
    assert not list((edgeenv_root / "failed-runs").glob("*"))
    ...
    assert shown["sampler_name"] == "jetson-tegrastats"
    assert shown["sample_count"] == 0
    assert shown["raw_artifacts"] == []
    assert "tegrastats unavailable" in shown["warnings"][0]
    assert shown["metadata"]["schema_version"] == "edgeenv.sampler-metadata.v1"
    assert shown["files"]["metadata"].endswith("sampler/metadata.json")


def test_cli_runs_sampler_show_fails_without_sampler_metadata(tmp_path):
    runner = CliRunner()
    script = tmp_path / "local_bench.py"
    script.write_text(
        """
...
""".strip()
    ...
    run_id = next((edgeenv_root / "runs").iterdir()).name
    show_result = runner.invoke(
        app,
        [
            "runs",
    ...
    assert "Sampler metadata not found for run" in show_result.output
